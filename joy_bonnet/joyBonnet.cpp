...
JoyBonnet::JoyBonnet()
...
#ifdef DEBUG
      std::cout << "Wiring Pi i2c setup failed:" << strerror(errno)
                << std::endl;
#endif
    }

    // Create and start the worker thread. Call run on the blocked service from
    // that thread.
    _worker_thread = new asio::thread(std::bind(
        static_cast<asio::io_context::count_type (asio::io_service::*)(void)>(
            &asio::io_context::run),
        &_io));

#ifdef DEBUG
    std::cout << "Debounce time out set at " << _time_out.count()
  ...
  void JoyBonnet::x_callback_falling(void) {
    ...
    instance._x_timer.async_wait([&](const asio::error_code& e) {
  ...
  void JoyBonnet::y_callback_falling(void) {
    ...
    // Setup a new async wait
    instance._y_timer.async_wait([&](const asio::error_code& e) {
  ...
  void JoyBonnet::a_callback_falling(void) {
    ...
    // Setup a new async wait
    instance._a_timer.async_wait([&](const asio::error_code& e) {
  ...
  void JoyBonnet::b_callback_falling(void) {
    ...
    // Setup a new async wait
    instance._b_timer.async_wait([&](const asio::error_code& e) {
  ...
  void JoyBonnet::start_callback_falling(void) {
    ...
    // Setup a new async wait
    instance._start_timer.async_wait([&](const asio::error_code& e) {
  ...
  void JoyBonnet::select_callback_falling(void) {
    ...
    // Setup a new async wait
    instance._select_timer.async_wait([&](const asio::error_code& e) {
  ...
  void JoyBonnet::p1_callback_falling(void) {
    ...
    // Setup a new async wait
    instance._p1_timer.async_wait([&](const asio::error_code& e) {
  ...
  void JoyBonnet::p2_callback_falling(void) {
    ...
    // Setup a new async wait
    instance._p2_timer.async_wait([&](const asio::error_code& e) {
...
#ifdef DEBUG
      std::cout << "Timer for " << get_pin_name(pin) << " has been cancelled"
                << std::endl;
#endif
    }
  }
  ...
  int JoyBonnet::read_joystick(int channel) {
    ...
    int configword =
        ADS1015_REG_CONFIG_CQUE_NONE | ADS1015_REG_CONFIG_CLAT_NONLAT |
        ...
        ADS1015_REG_CONFIG_GAIN_ONE | _channels[channel] |
        ADS1015_REG_CONFIG_OS_SINGLE;

    int configdata[2] = {configword >> 8, configword & 0xFF};
    int writeBytes = configdata[0] | configdata[1] << 8;
    if (wiringPiI2CWriteReg16(_i2cFileHandler, ADS1x15_POINTER_CONFIG,
                              writeBytes) == -1) {
      std::cout << "Wiring Pi Write16 Reg failed:" << strerror(errno)
                << std::endl;
    }

    int readValue =
        wiringPiI2CReadReg16(_i2cFileHandler, ADS1x15_POINTER_CONFIG);
    ...
    while (1) {
      ...
      if (((readValue >> 8) & 0xFF) & 0x80) {
        int analogdata =
            wiringPiI2CReadReg16(_i2cFileHandler, ADS1x15_POINTER_CONVERSION);

        analogdata = (analogdata >> 4) &
                     0xFFF;  // Gets 12 bit value and mask it just in case
